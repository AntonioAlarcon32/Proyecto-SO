...
int AddUsuario( ListaConectados *lista, char nick[20], int socket)	//Funcion para a̱adir un usuario a la lista
{
	if (lista->num == 10)
	{
		return 1;
...
int BuscarUsuario(ListaConectados *lista, char nickname[20] ) //Busca un usuario en la lista de conectados y si lo encu7entra devuelve un 1 si no devuelve un 0.
{
	int i = 0;
	int found = 0;
...
int Acceso(char mensaje[120],char nickname[20])			//Funcion para iniciar sesion
{
	...
	SesionIniciada = BuscarUsuario( &ListaConect , nick );
	p = strtok(NULL,",");	
	strcpy(password,p);
	char consulta[128];
